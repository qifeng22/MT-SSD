...
def main():
    ...
    with torch.no_grad():
        for idx, data_dict in enumerate(test_set):
            ...
            #data_dict['gt_boxes'].squeeze(dim=0)[:,0:6]= 
            gt = data_dict['gt_boxes'].squeeze(dim=0)
            gt[:,0:6] = gt[:,0:6]*6
            pred= pred_dicts[0]['pred_boxes']
            pred[:,0:6] = pred[:,0:6]*6
            print(list(data_dict.keys()))
            V.draw_scenes(
                gt_boxes=gt,points=data_dict['points'][:, 1:]*6, ref_boxes=pred,
