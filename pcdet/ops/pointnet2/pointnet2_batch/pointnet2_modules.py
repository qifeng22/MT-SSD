...
class PointnetSAModuleMSG_WithSampling(_PointnetSAModuleBase):
    ...
    def __init__(self, *,
        ...
        for i in range(len(radii)):
            ...
            for k in range(len(mlp_spec) - 1):
                ...
                #print(int(mlp_spec[1]/4))
                shared_mlps = []
                xinzeng_cc.extend([
                    nn.Conv2d(mlp_spec[k], int(mlp_spec[ 1]/4),
                              kernel_size=(1,nsample),bias=True),
                              nn.BatchNorm2d(int(mlp_spec[1]/4)),
                              nn.ReLU()
                ])


                shared_mlps.extend([
                    nn.Conv2d(mlp_spec[k]+int(mlp_spec[1]/4), mlp_spec[k + 1],
                              kernel_size=1, bias=False),
                    nn.BatchNorm2d(mlp_spec[k + 1]),
                    nn.ReLU()
                ])
                self.mlps.append(nn.Sequential(*shared_mlps))
                self.xinzeng.append(nn.Sequential(*xinzeng_cc))
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, cls_features: torch.Tensor = None, new_xyz=None, ctr_xyz=None):
        ...
        if len(self.groupers) > 0:
            for i in range(len(self.groupers)):
                new_features = self.groupers[i](xyz, new_xyz, features)  # (B, C, npoint, nsample)
                ns=new_features.size()[-1]
                for kkt in range(3):
                    zh = self.xinzeng[3*i+kkt](new_features)
                    zh2 = torch.cat((new_features,zh.repeat([1,1,1,ns])),1)
                    new_features=self.mlps[3*i+kkt](zh2)
